...
export default function ChartOverlay({ open, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex">
      ...
      <div className="ml-auto w-[420px] h-full bg-[#f8fafc] text-black shadow-xl relative z-10 p-3 flex">

        {/* FILTERS */}
        <div className={`${showFilters ? "w-1/3" : "w-[40px]"} border-r p-2`}>
          <div className="flex justify-between items-center mb-2">
            {showFilters && <h3 className="text-xs font-semibold">Filters</h3>}
            <button
              onClick={() => setShowFilters(!showFilters)}
              className="text-black bg-gray-50 text-xs px-1 focus:outline-none focus:ring-0 active:outline-none"
            >
              {showFilters ? "‹" : "›"}
            </button>
          </div>
          ...
          {showFilters && (
            <>
              <input
                placeholder="Search"
                className="w-full text-xs border px-2 py-1 mb-2 focus:outline-none focus:ring-0"
              />
        ...
        {/* VISUALS */}
        <div className={`${showVisuals ? "w-1/3" : "w-[40px]"} border-r p-2`}>
          <div className="flex justify-between items-center mb-2">
        ...
        {/* DATA */}
        <div className={`${showData ? "w-[34%]" : "w-[40px]"} p-3 flex flex-col h-full`}>
          <div className="flex justify-between items-center mb-2">
            {showData && (
              <h3 className="text-[12px] font-semibold text-gray-700">Data</h3>
            )}
            <button
          ...
          {showData && (
            <>
              <input
                placeholder="Search"
                className="w-full text-[11px] border rounded px-2 py-1 mb-3 outline-none focus:ring-0"
              />
        ...
        <button
          onClick={onClose}
          className="absolute top-2 right-2 text-xs bg-black text-white px-2 py-1 rounded focus:outline-none focus:ring-0"
        >
          Close
        </button>
