...
const CreateDashboard = ({ isOpen, onClose, onSuccess }) => {
  ...
  const handleNext = async () => {
    ...
    if (Object.keys(newErrors).length > 0) {
      ...
      if (!name.trim()) {
        setPopupError("Dashboard name is required");
      } else if (!description.trim()) {
        setPopupError("Description is required");
      } else if (!image) {
        setPopupError("Please upload an image");
      } else if (columns.length === 0) {
        setPopupError("Please add at least one column");
      }
  ...
  return (
    ...
    >
      ...
      >
        ...
        {popupError && (
          <div className="mb-4 flex justify-between items-center bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded-md">
